...
static void CAN_Mode_Config(void)
{
  ...
  RCC_APB1PeriphClockCmd(CAN_CLK, ENABLE);
	...
	CAN_InitStructure.CAN_Mode = CAN_Mode_Normal;  //正常工作模式CAN_Mode_LoopBack;//回环  
	CAN_InitStructure.CAN_SJW  = CAN_SJW_1tq;		   //BTR-SJW 重新同步跳跃宽度 2个时间单元
	 
	/* ss=1 bs1=4 bs2=2 位时间宽度为(1+4+2) 波特率即为时钟周期tq*(1+4+2)  */
	CAN_InitStructure.CAN_BS1  = CAN_BS1_12tq;		   //BTR-TS1 时间段1 占用了4个时间单元
	CAN_InitStructure.CAN_BS2  = CAN_BS2_1tq;		   //BTR-TS1 时间段2 占用了2个时间单元	
...
static void CAN_Filter_Config(void)
{
	...
	/*CAN筛选器初始化*/
	CAN_FilterInitStructure.CAN_FilterNumber = 1;	//筛选器组14
	CAN_FilterInitStructure.CAN_FilterMode   = CAN_FilterMode_IdMask;	//工作模式
	CAN_FilterInitStructure.CAN_FilterScale  = CAN_FilterScale_32bit;	//筛选器位宽为单个32位。
	/* 使能筛选器，按照标志的内容进行比对筛选，扩展ID不是如下的就抛弃掉，是的话，会存入FIFO0。 */

	CAN_FilterInitStructure.CAN_FilterIdHigh = 0X0000;//((((u32)0x201<<3)|CAN_ID_STD|CAN_RTR_DATA)&0xFFFF0000)>>16;		//要筛选的ID高位 
	CAN_FilterInitStructure.CAN_FilterIdLow  = 0X0000;//(((u32)0x201<<3)|CAN_ID_STD|CAN_RTR_DATA)&0xFFFF; //要筛选的ID低位 
...
//  TxMessage->ExtId=0x1314;					 //使用的扩展ID
  TxMessage->IDE   = CAN_ID_STD ;//标准模式        //CAN_ID_EXT;	//扩展模式
  TxMessage->RTR   = CAN_RTR_DATA;				 //发送的是数据
  TxMessage->DLC   = 8;							 //数据长度为8字节
	/*设置要发送的数据0-7*/
	TxMessage->Data[0] = (0x00FF >> 8);
	TxMessage->Data[1] = 0x00FF;
	TxMessage->Data[2] = (0x00AA >> 8);
	TxMessage->Data[3] = 0x00AA;
	TxMessage->Data[4] = (0x0001 >> 8);
	TxMessage->Data[5] = 0x0001;
	TxMessage->Data[6] = (0x0007 >> 8);
	TxMessage->Data[7] = 0x0007;
}
